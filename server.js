...
app.post('/api/place-order', upload.single('screenshot'), async (req, res) => {
  try {
    ...
    // ---------------------------------------------------------
    try {
      const userItemsHTML = orderData.cart.map(i => `
        <li>
      ...
      await brevoAPI.sendTransacEmail({
        ...
        htmlContent: `
          ...
          <p>
            ...
            We will contact you shortly for delivery updates.
          </p>

          <p>❤️ — Cloud 9 Pastries</p>
        `
      });
...
app.post('/api/products', upload.single('image'), async (req, res) => {
  ...
  if (req.body.options) {
    try {
      optionsArray = JSON.parse(req.body.options);

      if (!Array.isArray(optionsArray)) optionsArray = [];

      optionsArray = optionsArray.filter(v =>
...
app.put('/api/products/:id', upload.single('image'), async (req, res) => {
  try {
    ...
    if (req.body.options !== undefined) {
      try {
        let parsed = JSON.parse(req.body.options);
        if (!Array.isArray(parsed)) parsed = [];

        update.options = parsed.filter(v =>
          typeof v.label === "string" &&
          typeof v.price === "number"
        );
      } catch (err) {
